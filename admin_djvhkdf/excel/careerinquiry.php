...
$objPHPExcel->getActiveSheet()->setCellValue('A1', 'No')
							  ...
							  ->setCellValue('G1', 'Post Apply')
							  ->setCellValue('H1', 'IP')
							  ->setCellValue('I1', 'Sms Status')
...
if(mysqli_num_rows($res)>0)
{
	 ...
	 while($row = mysqli_fetch_object($res)) {
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A'.($i+1), $i)
									  ...
									  ->setCellValue('F'.($i+1), $row->dept)
									  ->setCellValue('F'.($i+1), $row->Post_Apply)
									  ->setCellValue('G'.($i+1), $row->ip)
									  ->setCellValue('H'.($i+1), $row->sms_status)
		 							  ->setCellValue('J'.($i+1), $timefield);	
