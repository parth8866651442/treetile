...
$sql ='UPDATE exportpdf SET status ="'.$_POST['status'].'",';
...
{	
	...
	else 
	{
		...
		$mpdf = '../uploads/exportpdf/'.$pdf;
		move_uploaded_file($_FILES['pdf']['tmp_name'],$mpdf); 
		
		 
		 if(file_exists('../uploads/exportpdf/'.$_POST['old_pdf']))
			unlink('../uploads/exportpdf/'.$_POST['old_pdf']);
