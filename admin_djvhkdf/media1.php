...
<?php 
	...
	/*if($obj_fun->getMetaData('news-with-multiple-image-and-detail') == 0 or $_SESSION['type'] == 'admin')
	...
	<div class="container">
		...
		<div class="content-block" role="main">
			<ul class="breadcrumb">
				...
				<li class="active">Exhibition & Event</li>
			</ul>
			<div class="row-fluid">
				<article class="span12 data-block nested">
					<div class="data-container">
						<header>
							<h2>Event - ( for better resolution keep Image size <?php echo news_big_width;?> x <?php echo news_big_height;?> px )</h2>
						</header>
						<section class="tab-content">
							...
							<?php
								if(isset($_POST['submit']) && $_POST['submit'] !='' && $_POST['submit'] =='Save')
								{
									...
									$arg['slug'] = $obj_fun->createSlug($title);
									$arg['details'] =  htmlspecialchars($details);
									$arg['yturl'] = $yturl;
									
									
									$arg_img['news_id'] =  $LastInsertId[0][0];
									...
									if($s_ins > 0)
									{
							            ...
							            $LastInsertId = $obj_fun->select_all_record($Query);
										$arg_img['news_id'] =  $LastInsertId[0][0];
										...
										if($s_ins>0)
										{
											echo '<script>success("Event-Detail Insert successfully.")</script>';
											echo "<script>urlRefresh('media-view1.php', 3000);</script>";
										}
										else
											echo '<script>danger("Get error while updating Event Details	 !")</script>';
									}
								}
								if(isset($_POST['submit']) && $_POST['submit'] !='' && $_POST['submit'] =='Update')
								{
									...
									if($s_ins > 0)
									{
										...
										if($s_ins>0)
										{
											echo '<script>success("Event-Detail Update successfully.")</script>';
											echo "<script>urlRefresh('media1.php?action=edit&id=".$n_id."', 3000);</script>";
										}
										else
											echo '<script>danger("Get error while updating Event-Detail  !")</script>';
									}
								}
							?>
								<form id="form" class="form-horizontal" method="post" enctype="multipart/form-data" onSubmit="return news_validate();" action="#">
									
                                   <input id="model_no" name="type" class="input-xxlarge" type="hidden" value="event" />
									...
									<div class="control-group">
										<label class="control-label" for="input" style="width: 80px; text-align: left;">Date :</label>
										<div class="controls">
											<input id="model_no" name="pdate" class="input-xxlarge" type="date" value="<?php echo $pdate; ?>" />
										</div>
									</div>

									<!-- <div class="control-group">
										<label class="control-label" for="input" style="width: 80px; text-align: left;">City :</label>
										<div class="controls">
											<input id="city" name="city" class="input-xxlarge" type="text" value="<?php echo $city; ?>" />
										</div>
									</div> -->
							...
							<form class="form-gallery" action="#" method="post">
								...
								<header>
									...
									<ul class="data-header-actions tabs">
										...
										<li class="demoTabs"><input type="submit" onClick="return deleteAlert();"  name="delete" id="delete" value="Delete" class="btn btn-alt btn-primary" /></li>
									</ul>
	...
	<script type="text/javascript">
		$(function() {
			$("#uploader").plupload({
				...
				url : 'plupload/upload.php?folder=media&tw=<?php echo news_th_width; ?>&th=<?php echo news_th_height; ?>',
...
function deleteAlert(){
	...
	<script type="text/javascript">
		jQuery(document).ready(function($) {
			...
			$('.dynamic-menu #media').addClass('active');
			$('.dynamic-menu #m_new').addClass('active');
		});
	</script>
