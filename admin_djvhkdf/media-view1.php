...
<?php 
	$sql = "select * from news where type='event' ORDER BY `order_news` ASC";
	...
	<div class="container">
		...
		<div class="content-block" role="main">
			<ul class="breadcrumb">
				...
				<li class="active">View Exhibition & Event</li>
			</ul>
			<div class="row-fluid">
								...
								<table class="datatable table table-striped table-bordered" id="catalogueview" align="center">
									...
									?>
										<tr class="odd gradeX" id="<?php echo $c['id']; ?>">
											...
											<td><a href="media1.php?action=edit&id=<?php echo $c['id']; ?>" class="btn btn-alt btn-primary">View</a>			
...
<script type="text/javascript">
	 jQuery(document).ready(function($) {
		...
		$('.dynamic-menu #m_view').addClass('active');
	});
</script>
	...
	$(document).ready(function() {
		...
		$('#catalogueview').tableDnD({
			onDrop: function(table, row) {
				...
				$.post("custom-ajax.php",{action:'event_order_change',order:order },function(result){
					//console.log(result);
					if(result>0)
						location.reload();
				});
