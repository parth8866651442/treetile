...
<?php 
	$sql = "select * from news where type='news-with-one-image-and-detail' ORDER BY `order_news` ASC";
								...
								<table class="datatable table table-striped table-bordered" id="catalogueview" align="center">
									<thead>
										<tr>
											...
											<th><input type="checkbox" value="Check All" id="selecctall" name="selecctall" style="left: -16px; top: 16px;" /></th></th>
											<th>Title</th>
											<th>Description</th>
											<th>Image</th>
											<th>Date</th>
									...
									?>
										<tr class="odd gradeX" id="<?php echo $c['id']; ?>">
											...
											<td><img src="../uploads/news/<?php echo $c['image']; ?>" width="150" /></td>
											<td><?php echo date("d/m/Y", strtotime($c['date']));  ?></td>
											<td><a href="news.php?action=edit&id=<?php echo $c['id']; ?>" class="btn btn-alt btn-primary">View</a>
											</td>
...
<script type="text/javascript">
	 jQuery(document).ready(function($) {
		...
		$('.dynamic-menu #oview').addClass('active');
	});
</script>
	...
	$(document).ready(function() {
		...
		$('#catalogueview').tableDnD({
			onDrop: function(table, row) {
				...
				$.post("custom-ajax.php",{action:'our_news',order:order },function(result){
					//console.log(result);
					if(result>0)
						location.reload();
				});
					...
					if(chkLength >= 1)
					{
						if (isConfirm) {
							$('input[name="img_delete"]:checked').each(function() {
							   $.post("custom-ajax.php",{action:'Delete_News',id:this.value },function(result){
							   });
							});
