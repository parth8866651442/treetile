...
if(isset($_GET['id']) && isset($_GET['action']) && $_GET['id']!='') 
{
	...
	//$nsql = "SELECT * FROM news_video where news_id = ".$id;
	//$news_video_data = $obj_fun->getRecords($nsql); 
...
<body class="fixed-layout">
			...
			<div class="row-fluid">
				<article class="span12 data-block nested">
					<div class="data-container">
						<header>
							<h2>Blog - ( for better resolution keep Image size <?php echo news_big_width;?> x <?php echo news_big_height;?> px )</h2>
						</header>
							...
							<?php
								if(isset($_POST['submit']) && $_POST['submit'] !='' && $_POST['submit'] =='Save')
								{
									...
									if($s_ins > 0)
									{
										...
										if($s_ins>0)
										{
											echo '<script>success("Blog-Detail Insert successfully.")</script>';
											echo "<script>urlRefresh('viewblog.php', 3000);</script>";
										}
										else
											echo '<script>danger("Get error while updating Blog Details	 !")</script>';
									}
								}
								if(isset($_POST['submit']) && $_POST['submit'] !='' && $_POST['submit'] =='Update')
								{
									...
									if($s_ins > 0)
									{
										...
										if($s_ins>0)
										{
											echo '<script>success("Blog-Detail Update successfully.")</script>';
											echo "<script>urlRefresh('media1.php?action=edit&id=".$n_id."', 3000);</script>";
										}
										else
											echo '<script>danger("Get error while updating Blog-Detail  !")</script>';
									}
								}
							?>
								<form id="form" class="form-horizontal" method="post" enctype="multipart/form-data" onSubmit="return news_validate();" action="#">
									
                                   <input id="model_no" name="type" class="input-xxlarge" type="hidden" value="blog" />
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			$('#selecctall').click(function(Blog) {  //on click
	...
	<script type="text/javascript">
	 jQuery(document).ready(function($) {
		...
		$('.dynamic-menu #b_new').addClass('active');
	});
</script>
</body>
</html>
