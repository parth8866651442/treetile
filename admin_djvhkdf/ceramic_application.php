...
<?php 
	...
	$folder = '../uploads/applicationlogo/';
	if (!file_exists($folder)) {
			@mkdir($folder, 0777, true);
		}
	$color = $obj_fun->getMetaData('applicationlogo');
	...
	<div class="container">
		...
		<div class="content-block" role="main">
			<ul class="breadcrumb">
				...
				<li class="active">Add Ceramic Application</li>
			</ul>
			<div class="row-fluid">
				<article class="span12 data-block nested">
					<div class="data-container">
						<header>
							<h2>Ceramic Application <?php echo $color; ?></h2>
							<br><br>
							<hr>
						</header>
                            ...
                            <?php 
								if(isset($_POST['submit']) && $_POST['submit'] == 'create'){ 
									if ($color == 1 ) {
										...
										move_uploaded_file($_FILES[ 'image' ][ 'tmp_name' ], $a );
										$s_ins = $obj_fun->insertceramicapplication($_POST,$a);
										//print_r($s_ins); die;
										
									} else {
										$s_ins = $obj_fun->insertceramicapplication($_POST['name'],'',$_POST['status']);
									}

									
									echo '<script>success("Application Insert successfully.")</script>';
									echo "<script>urlRefresh('ceramic_application.php');</script>";
								}
								if(isset($_POST['submit']) && $_POST['submit'] == 'update' && isset($_GET['id']) && $_GET['id'] != ''){ 
								
								 	$psdata = $obj_fun->getceramicapplicationdata($_GET['id']);
									$path_parts = pathinfo($_FILES['image']['name']);
									$fileExtension = $path_parts['extension'];
									$a = $folder.randomString(6).'.'.$fileExtension;
									move_uploaded_file($_FILES[ 'image' ][ 'tmp_name' ], $a );
									 
									$obj_fun->updateceramicapplication($_POST,$a,$_GET['id']);
									echo '<script>success("Application Update successfully.")</script>';
									echo "<script>urlRefresh('ceramic_application.php');</script>";
								}
								?>
							...
							<form id="myForm" class="form-horizontal" method="post" action="#" enctype="multipart/form-data">
								...
								<?php }	?>
									...
									<div class="control-group" >
										<label for="fileInput" class="control-label">Application Name</label>	
										...
										<div class="controls">
											<div data-provides="fileupload" class="fileupload fileupload-new">
												<div class="input-append">
													...
													</div><span class="btn btn-alt btn-file"><span class="fileupload-new">Select Image</span><span class="fileupload-exists">Change</span>
			...
			<?php $data = $obj_fun->getceramicapplicationname();
				if(isset($data) && $data !=''){
			?>						
			<div class="row-fluid">
										...
										<table class="datatable table table-striped table-bordered" id="catalogueview" align="center">
											...
											<tbody>
												...
												<tr class="odd gradeX" id="<?php echo $v['id']; ?>">
													...
													<center>
														<td><img src="<?php echo $v['app_img']; ?>" style="width: 50px;"/></td> </td>
													</center>
                                                    <th>
                                                    <?php 
														if($v['status'] == 0){ 
													...
													<td align="center">
														<a href="ceramic_application.php?action=update&id=<?php echo $v['id']; ?>" class="btn btn-alt btn-primary" >Edit</a>										
	...
	<script>
		function deleteAlert( id ) {
			swal( {
					...
					text: "All Data will be remove in this Ceramic Product Application.",
					type: "warning",
				...
				function ( isConfirm ) {
					if ( isConfirm ) {
						$.post( "custom-ajax.php", {
							...
							swal( "Deleted!", "Ceramic Product Application", "success" );
							location.reload();
						} );
					} else {
...
<script type="text/javascript">
	 jQuery(document).ready(function($) {
		$('.dynamic-menu #ceramic_application').addClass('active');
	});
</script>
...
$(document).ready(function() {
	...
	$('#catalogueview').tableDnD({
		onDrop: function(table, row) {
			...
			$.post("custom-ajax.php",{action:'application_order_change',sequence_order:sequence_order },function(result){
				//console.log(result);
				if(result>0)
					location.reload();
			});
...
function deleteAlertmega() {
			swal( {
					title: "Are you sure want to delete Checked Ceramic Product Application?",
					text: "All Data will be remove in this Checked Ceramic Product Application.",
					type: "warning",
					showCancelButton: true,
					confirmButtonColor: "#DD6B55",
					confirmButtonText: "Yes, Delete!",
				...
				function ( isConfirm ) {
					...
					if ( chkLength >= 1 ) {
						if ( isConfirm ) {
							...
							swal( {
									title: "Deleted!",
									text: "Ceramic Product Application Deleted",
									type: "success"
								},
								function () {
									location.reload();
