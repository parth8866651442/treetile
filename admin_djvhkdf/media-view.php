...
<?php 
	$sql = "SELECT * FROM `news` WHERE `type` LIKE '%News-With-Video%' ORDER BY `order_news` ASC";
...
<body class="fixed-layout">
	<div class="container">
		...
		<div class="content-block" role="main">
			<ul class="breadcrumb">
				...
				<li class="active">View News With Video </li>
			</ul>
			<div class="row-fluid">
								...
								<table class="datatable table table-striped table-bordered" id="catalogueview" align="center">
									...
									?>
										<tr class="odd gradeX" id="<?php echo $c['id']; ?>">
											...
											<td><a href="media.php?action=edit&id=<?php echo $c['id']; ?>" class="btn btn-alt btn-primary">View</a></td>
	...
	$(document).ready(function() {
		...
		$('#catalogueview').tableDnD({
			onDrop: function(table, row) {
				...
				$.post("custom-ajax.php",{action:'news_with_video',order:order },function(result){
					//console.log(result);
					if(result>0)
						location.reload();
				});
	...
	<script type="text/javascript">
	 jQuery(document).ready(function($) {
		...
		$('.dynamic-menu #vnews').addClass('active');
	});
	</script>
	<script>
					...
					if(chkLength >= 1)
					{
						if (isConfirm) {
							$('input[name="img_delete"]:checked').each(function() {
							   $.post("custom-ajax.php",{action:'Delete_News_With_Video',id:this.value },function(result){
							   });
							});
