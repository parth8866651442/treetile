...
<body class="fixed-layout">
	<div class="container">
		...
		<div class="content-block" role="main">
			...
			<div class="row-fluid">
				<article class="span12 data-block nested">
					<div class="data-container">
						<header>
							<h2>Add Advertisement Photos ( for better resolution keep Image size <?php echo advertisement_big_width;?> x <?php echo advertisement_big_height;?> px )</h2>
							<br><br>
							<hr>
						</header>
								...
								if(isset($_POST['submit']) && $_POST['submit'] !='' && $_POST['submit'] =='Create')
								{
									...
									{ 
										...
										{	
											...
											$res = $obj_fun->insertGallery('advertisement',$image,$thumb);
											echo "<script>urlRefresh('advertisement.php');</script>";
										}
									}
								}
								?>
								<form id="form" class="form-horizontal" method="post" enctype="multipart/form-data" action="#">
									<input type="hidden" name="img_path" id="img_path" value="uploads/advertisement" />
			...
			<?php
			$gallery = $obj_fun->getGallery('advertisement');
	...
	$(function() {
		$("#uploader").plupload({
			...
			url : 'plupload/upload.php?folder=../../uploads/advertisement&tw=<?php echo advertisement_th_width;?>&th=<?php echo advertisement_th_height;?>',
			max_file_count: 20,
			chunk_size: '1mb',
			unique_names : true,
			resize : {
				width : <?php echo advertisement_big_width;?>, 
				height : <?php echo advertisement_big_height;?>, 
				quality : 95,
...
<script type="text/javascript">
	 jQuery(document).ready(function($) {
		...
		$('.dynamic-menu #advertisement').addClass('active');
	});
</script>
...
$(document).ready(function() {
	...
	$('#catalogueview').tableDnD({
		onDrop: function(table, row) {
			...
			$.post("custom-ajax.php",{action:'advertisement_order_change',order:order },function(result){
				//console.log(result);
				if(result>0)
					location.reload();
			});
