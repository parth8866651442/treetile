...
<body class="fixed-layout">
	<div class="container">
		...
		<div class="content-block" role="main">
			<ul class="breadcrumb">
				...
				<li>Image Gallery</li>
				<li class="active">Live Photos</li>
			</ul>
			<div class="row-fluid">
				<article class="span12 data-block nested">
					<div class="data-container">
						<header>
							<h2>Add Live Photos ( for better resolution keep Image size <?php echo showroom_big_width;?> x <?php echo showroom_big_height;?> px )</h2>
							<br><br>
							<hr>
						</header>
								...
								if(isset($_POST['submit']) && $_POST['submit'] !='' && $_POST['submit'] =='Create')
								{
									...
									{ 
										...
										{	
											...
											$res = $obj_fun->insertGallery('showroom',$image,$thumb,'');
											echo "<script>urlRefresh('showroom.php');</script>";
										}
									}
								}
								?>
								<form id="form" class="form-horizontal" method="post" enctype="multipart/form-data" action="#">
									<input type="hidden" name="img_path" id="img_path" value="uploads/showroom" />
			...
			<?php
			$gallery = $obj_fun->getGallery('showroom');
	...
	$(function() {
		$("#uploader").plupload({
			...
			url : 'plupload/upload.php?folder=../../uploads/showroom&tw=<?php echo showroom_big_width;?>&th=<?php echo showroom_big_height;?>',
			max_file_count: 20,
			chunk_size: '1mb',
			unique_names : true,
			resize : {
				width : <?php echo showroom_big_width;?>, 
				height : <?php echo showroom_big_height;?>, 
				quality : 95,
...
<script type="text/javascript">
	 jQuery(document).ready(function($) {
		...
		$('.dynamic-menu #showroom').addClass('active');
	});
</script>
...
$(document).ready(function() {
	...
	$('#catalogueview').tableDnD({
		onDrop: function(table, row) {
			...
			$.post("custom-ajax.php",{action:'showroom_order_change',order:order },function(result){
				//console.log(result);
				if(result>0)
					location.reload();
			});
