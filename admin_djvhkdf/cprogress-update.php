...
$errors=0;
$image = '';
$newwidth= certificate_th_width;
$sql ='UPDATE certificate SET name ="'.$_POST['name'].'", status ="'.$_POST['status'].'",';
...
if (!empty($_FILES['image']) && $_FILES['image']['name'] !='' && is_uploaded_file($_FILES['image']['tmp_name'])) 
{
	...
	else
	{
		...
		$thumb_width= certificate_th_width; // Fix the width of the thumb nail images
		$thumb_height= certificate_th_height; // Fix the height of the thumb nail imaage
		...
		if($_FILES['image']['type']=="image/jpeg" || $_FILES['image']['type']=="image/jpg"){
			...
			if(file_exists('../uploads/certificate/'.$_POST['old_image']))
				unlink('../uploads/certificate/'.$_POST['old_image']);
...
{	
	...
	else 
	{
		...
		$mpdf = '../uploads/certificate/'.$pdf;
		move_uploaded_file($_FILES['pdf']['tmp_name'],$mpdf); 
		
		 
		 if(file_exists('../uploads/certificate/'.$_POST['old_pdf']))
			unlink('../uploads/certificate/'.$_POST['old_pdf']);
